...
def preprocess_qwen_2_visual(
    ...
    for source in sources:
        ...
        for conv in source:
            ...
            def replace_token(token_str, grid_list, idx_counter):
                nonlocal content
                if token_str not in content:
                    return
                ...
                for i in range(len(parts) - 1):
                    ...
                    replacement = (
                        "<|vision_start|>"
                        + ("<|image_pad|>" * grid_list[idx_counter])
                        + "<|vision_end|>"
                    )
            ...
            visual_idx_img   = replace_token("<image>",     grid_thw_image, visual_idx_img)   or visual_idx_img
            visual_idx_vid   = replace_token("<video>",     grid_thw_video, visual_idx_vid)   or visual_idx_vid
            visual_idx_focal = replace_token("<focal_crop>", grid_thw_focal, visual_idx_focal) or visual_idx_focal

            conv_msg = [{"role": role, "content": content}]
            encode_id = tokenizer.apply_chat_template(conv_msg)
            input_id += encode_id
            target += [IGNORE_INDEX] * len(encode_id) if role in ["user", "system"] else encode_id
...
class LazySupervisedDataset(Dataset):
    ...
    def process_image_unified_448(self, image_file):
        processor = copy.deepcopy(self.data_args.image_processor)
        image = Image.open(image_file).convert("RGB")
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            if isinstance(image_file, List):
                if len(image_file) > 1:
                    ...
                    results = [self.process_image_unified_448(f) for f in image_file]
                    image, grid_thw = zip(*results)
                else:
                    image_file = image_file[0]
                    image_file = os.path.join(image_folder, image_file)
                    image, grid_thw = self.process_image_unified_448(image_file)
                    image = [image]
            else:
                image_file = os.path.join(image_folder, image_file)
                image, grid_thw = self.process_image_unified_448(image_file)
        ...
        if "focal_crop" in sources[0]:
            focal_path = os.path.join(sources[0]["data_path"], sources[0]["focal_crop"])
            focal_image, focal_grid = self.process_image_unified(focal_path)
            grid_thw_focal = [focal_grid.prod() // self.data_args.image_processor.merge_size**2]
        ...
        data_dict = preprocess_qwen_2_visual(
            chat_sources,
            self.tokenizer,
            grid_thw_image=grid_thw_merged or None,
            grid_thw_video=video_grid_thw_merged or None,
            grid_thw_focal=grid_thw_focal,
        )

        position_ids, _ = self.get_rope_index(
            self.data_args.image_processor.merge_size,
            data_dict["input_ids"],
            image_grid_thw=torch.stack(grid_thw, dim=0) if isinstance(grid_thw, list) and len(grid_thw) > 0 else (grid_thw.unsqueeze(0) if grid_thw is not None else None),
            video_grid_thw=torch.stack(video_grid_thw, dim=0) if isinstance(video_grid_thw, list) and len(video_grid_thw) > 0 else (video_grid_thw.unsqueeze(0) if video_grid_thw is not None else None),
            focal_crop_grid_thw=torch.stack([focal_grid], dim=0) if "focal_crop" in sources[0] and focal_grid is not None else None,
            second_per_grid_ts=second_per_grid_ts if second_per_grid_ts else None,
        )

        if "image" not in sources[0] and "video" not in sources[0] and "focal_crop" not in sources[0]:
        ...
        if "focal_crop" in self.list_data_dict[i]:
            data_dict["focal_crop"] = focal_image.unsqueeze(0)
            data_dict["focal_crop_grid_thw"] = focal_grid.unsqueeze(0)
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        focal_crop_grids = [
            instance["focal_crop_grid_thw"]
            for instance in instances
            if "focal_crop_grid_thw" in instance
        ]
        from torchvision.transforms import functional as F

        if len(focal_crops) != 0:
            batch["focal_crop"] = torch.cat(focal_crops, dim=0)
            batch["focal_crop_grid_thw"] = torch.cat(focal_crop_grids, dim=0)
        else:
